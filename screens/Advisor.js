...
const Transactions = () => {
  ...
  const [Advisor, setAdvisor] = React.useState( [{"Name":"","Title":"","Content":"","Email":""}]);

  

  React.useEffect(() => {
    const interval = setInterval(() => {
      fetch("http://192.168.1.105:80/pangasolo/getAdvisor.php")
      .then((response) => response.json())
      .then((data) => {
        setAdvisor(data.advisorList);
        console.log(data.advisorList);
      })
      .catch((error) => {
  ...
  const renderItem = ({ item }) => (
    ...
    >
      ...
      >
        <Text className=" font-semibold text-base text-blue-100 capitalize">{item.Name}</Text>
      ...
      <Text className="font-semibold text-sm text-blue-100 capitalize" >{item.content}</Text>
        <Text className="font-semibold text-xs text-blue-300 capitalize">{item.Email}</Text>
      </View>
      <View>
